    ...
    └── profiles (1:1)
            ...
            └── children (1:N)
                    ...
                    └── milestones (1:N)
                            ...
                            (is_custom = false, source≠user → AI-generated checklist item)
                            (is_custom = false, source=user → user-added challenge)
```
...
Stores three types of milestone records in a single table, distinguished by `is_custom`
and `source`:

| Type | `is_custom` | `source` | Description |
|------|-------------|----------|-------------|
| Custom milestone | `true` | `NULL` | Parent-logged diary entry with date, description, location |
| AI-generated item | `false` | `'WHO'` / `'CDC'` / `'AAP'` | Generated by Claude, shown in AI-sjekkliste |
| User-added challenge | `false` | `'user'` | Manually added by parent on AI-sjekkliste page; treated as extra challenge for the child's age band |

Unified table avoids duplication of the `is_checked` toggle logic.
...
- AI items (`is_custom = false`, `source` ≠ `'user'`): `date` is NULL; `age_band` and
  `source` (`'WHO'`/`'CDC'`/`'AAP'`) MUST be present.
- User-added challenges (`is_custom = false`, `source = 'user'`): `date` is NULL;
  `age_band` MUST be set to the child's current age band at time of creation; `title`
  MUST be non-empty.
